...
export default function Kategori_user() {
    ...
    return <>
        <AppBar title={state["kategori"]} leadingButton={true} url={"back"} />
        <Height height={120} />
                            ...
                            <ul class="list-group" style={{ borderRadius: "0px" }}>
                                ...
                                {kategori.map((list, index) => (
                                    <li style={{ padding: "5px 10px 5px 10px" }} class="list-group-item d-flex justify-content-between align-items-center">
                                        <Link {...list["id_kategori"] == id && { className: "menu-aktif" }} to={"/" + list["id_kategori"] + "/kategori/" + Url_encoded((list["nama_kategori"]).toLowerCase()) + ".html"}
